FUNCTION NumTrim, NUMBER, FORMAT, FLAG, QUIET=quiet
;+
; Project     : SOHO - CDS
;
; Name        : 
...
;
	IF S(0) GT 0 THEN BEGIN
	    ...
	    IF NP GT 1 THEN OUT = MAKE_ARRAY(/STRING,DIMENSION=S(1:S(0))) ELSE OUT = 'String'
	    CASE N_PARAMS() OF
		1: FOR I=0L,NP-1 DO OUT(I) = NumTrim(NUMBER(I),QUIET=quiet)
		2: FOR I=0L,NP-1 DO OUT(I) = NumTrim(NUMBER(I),QUIET=quiet,FORMAT)
		3: FOR I=0L,NP-1 DO OUT(I) = NumTrim(NUMBER(I),QUIET=quiet,FORMAT,FLAG)
	    ENDCASE
	    RETURN,OUT
	ENDIF
;
...
;
	IF (TYPE EQ 6) OR (TYPE EQ 9) THEN BEGIN
		...
		CASE N_PARAMS() OF
			1:  BEGIN
				RNUMBER = NumTrim(RNUMBER)
				INUMBER = NumTrim(INUMBER)
				END
			2:  BEGIN
				RNUMBER = NumTrim(RNUMBER,FORMAT)
				INUMBER = NumTrim(INUMBER,FORMAT)
				END
			3:  BEGIN
				RNUMBER = NumTrim(RNUMBER,FORMAT,FLAG)
				INUMBER = NumTrim(INUMBER,FORMAT,FLAG)
				END
		ENDCASE

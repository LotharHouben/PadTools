PRO ReportArrayStatistics, DIRECTDATAP=directdatap
;;
...
m=Moment(*ppd, MDEV=mdev, SDEV=sdev)
mmin=Min(*ppd,MAX=mmax)
;; printtocon also accepts string arrays, each of the elements will be
;; printed on a single line
s=STRARR(10)
s[0]="% ReportArrayStatistics: "+(*pp).id
s[1]="  Min:                     " + MyString(mmin)
s[2]="  Max:                     " + MyString(mmax)
s[3]="  Mean:                    " + MyString(m[0])
s[4]="  Sum:                     " + MyString(Total(*ppd, /DOUBLE))
s[5]="  Variance:                " + MyString(m[1])
s[6]="  Standard deviation:      " + MyString(sdev)
s[7]="  Mean absolute deviation: " + MyString(mdev)
s[8]="  Skewness:                " + MyString(m[2])
s[9]="  Kurtosis:                " + MyString(m[3])

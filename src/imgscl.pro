...
;
    ...
    EndIf Else If ( Keyword_Set(exp_scl) ) Then Begin
       scaling_type = 4
    EndIf Else If ( Keyword_Set(gamma_scl) ) Then Begin
       scaling_type = 5
       gamma=FLOAT(gamma_scl)
...
;
	...
	3: Begin
	    ...
	    tmp = ((Temporary(tmp)/(max_lvl-min_lvl))^(1./gamma))*(max_lvl - min_lvl)
	    image = BytScl(tmp, Top=top-1B) + 1B
         End
        5: Begin
            print, "gamma scaling with gamma=", gamma
	    tmp =  (array <max_lvl >min_lvl) - min_lvl
	    tmp = ((Temporary(tmp)/(max_lvl-min_lvl))^(1./gamma))*(max_lvl - min_lvl)
	    image = BytScl(tmp, Top=top-1B) + 1B
	End
